...
def create_spark_session():
    ...
    spark = (
        SparkSession.builder.appName("PySpark Iceberg with MinIO")
        ...
        .config("spark.hadoop.fs.s3a.endpoint", "http://minio:9000")   # URL de ton MinIO
        .config("spark.hadoop.fs.s3a.access.key", "minioadmin")        # ton access key
        .config("spark.hadoop.fs.s3a.secret.key", "minioadmin")        # ton secret key
        .config("spark.hadoop.fs.s3a.path.style.access", "true")       # important pour MinIO
        .getOrCreate()
    )
...
def save_to_iceberg(spark, df):
    ...
    try:
        # Create database in the 'minio' catalog
        spark.sql("CREATE DATABASE IF NOT EXISTS minio.sample_db")

        # Write to Iceberg table
        df.write.format("iceberg").mode("overwrite").saveAsTable(
            "minio.sample_db.employees"
        )

        print("✅ Data successfully saved to Iceberg table: minio.sample_db.employees")

        # Verify the save
        print("\nReading back from Iceberg table:")
        iceberg_df = spark.table("minio.sample_db.employees")
